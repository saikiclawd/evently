      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-200">
            <h3 className="text-sm font-bold text-gray-900">Recent Wishlist Submissions</h3>
          </div>
          {isLoading ? (
            ...
            <div className="py-12 text-center text-gray-500">
              ...
              <p className="text-xs text-gray-400 mt-1">Wishlists will appear here when clients submit from your website</p>
            </div>
          ) : (
            wishlists.map((w) => (
              <div key={w.id} className="flex items-center gap-3.5 px-5 py-3.5 border-b border-gray-200 last:border-0">
                <div className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center flex-shrink-0">
                  <BookOpen size={16} className="text-blue-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-gray-900">{w.visitor_name}</div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h3 className="text-sm font-bold text-gray-900 mb-5">Website Integration</h3>
          <div className="space-y-3">
            {settings.map((s, i) => (
              <div key={i} className="flex items-center gap-3.5 p-3.5 bg-gray-50 rounded-xl border border-gray-200">
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  s.enabled ? "bg-green-50" : "bg-white"
                }`}>
                  <s.icon size={16} className={s.enabled ? "text-green-600" : "text-gray-500"} />
                </div>
                <div className="flex-1">
                  <div className="text-sm font-semibold text-gray-900">{s.label}</div>
                  <div className="text-xs text-gray-500">{s.desc}</div>
                </div>
                <div className={`w-10 h-[22px] rounded-full p-0.5 cursor-pointer transition-colors ${s.enabled ? "bg-green-500" : "bg-gray-100"}`}>
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
            <div className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide mb-2">Embed Code</div>
            <code className="text-[11px] text-teal-600 font-mono block break-all leading-relaxed">
              {'<script src="https://evently.yourdomain.com/widget.js" data-company="YOUR_ID"></script>'}
            </code>
            <p className="text-[11px] text-gray-400 mt-2">Add this to your website to enable the wishlist widget</p>
          </div>
        </div>
      </div>
    </div>
